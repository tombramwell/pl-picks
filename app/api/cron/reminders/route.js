...
// 3. Get all active managers (Anyone who has ever made a pick!)
    ...
    for (const email of uniqueEmails) {
      ...
      if (missingMatches.length > 0) {
        ...
        // Safely attempt to send the email so one failure doesn't stop the rest
        try {
          await resend.emails.send({
            from: 'Premiership Picks <premiershippicks@tombramwell.com>', 
            to: email, // <--- Now dynamically targets anyone who has played!
    ...
    return NextResponse.json({ success: false, error: 'Error syncing picks' }, { status: 500 });
  }
}