...
/*
     ...
     * --------------------------------------------------
     *
     * Only run this if EVERY club was successfully processed.
     */

    if (stats.clubsSuccessful === teams.length && stats.clubsFailed === 0) {
      const inactiveBefore = new Date();

      // THE FIX: Subtract the buffer days so we only target genuinely stale players!
      inactiveBefore.setDate(inactiveBefore.getDate() - INACTIVE_AFTER_DAYS);
      ...
      if (stalePlayers.length > 0) {
        const staleIds = stalePlayers.map((player) => player.plId);
        ...
        stats.playersMarkedInactive = result.modifiedCount;
      }
    } else {
      stats.errors.push(
        'Inactive-player cleanup skipped because one or more clubs failed to sync.'
      );
    }
